...
def get_type(type):
    value = f'htpps://openebench.bsc.es/bioschemas/oebagents#{type}'
    return value

def get_identifier(meta):
    value = f"https://openebench.bsc.es/bioschemas/agents/observatory:{meta.get('name')}:{meta.get('version')}/{meta.get('type')}"
    return value
...
def build_json_ld(meta):
    """
    Build JSON-LD from agent metadata
    """
    print(meta)
    context =  {

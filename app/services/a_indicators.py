    ...
    for doc in documentation:
        if doc.type.lower() in installation_types:
            ...
            if is_url_operational(doc.url):
                ...
                logs.append(f"❌ Installation instructions URL is not operational.")

    if installation_instructions:
        logs.append("❌ Installation instructions are available but the URL is not operational. Checking sources ...")
    else:
...
def compA1_4(instance) -> Tuple[bool, List[str]]:
    ...
    logs.append("⚙️ Verifying if test data is explicitly provided and if the provided URL is operational.")
    ...
    for doc in instance.documentation:
        if doc.type.lower() == 'test data':
            if is_url_operational(doc.url):
                ...
                logs.append(f"✅ Test data is available in documentation and URL operational. URL: {doc.url}")
            else:
    ...
    if has_test_data_in_docs:
        ...
        logs.append("❌ No test data found in documentation or the provided URL is not operational.")
        logs.append("Result: FAILED")
        return False, logs
        ...
        for url in webpage:
            if any(e in str(url) for e in E_INFRASTRUCTURES):
                ...
                if is_operational:
                    ...
                    logs.append(f"❌ The following free e-infrastructure URL is not operational: {url}")
            else:
...
def compA3_5(instance) -> Tuple[bool, List[str]]:
    ...
    logs.append("⚙️ Checking if more than one e-infrastructure is referenced in the links and the link is operational")
    webpage = instance.webpage or []
    logs = log_webpages(instance, logs)

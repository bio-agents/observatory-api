...
def test_compI2_2_with_agentshed_source():
    instance = MockInstance(source=['agentshed'])
    result, logs = compI2_2(instance)
    assert result == True

def test_compI2_2_with_galaxy_and_agentshed_sources():
    instance = MockInstance(source=['galaxy', 'agentshed'])
    result, logs = compI2_2(instance)
    assert result == True
